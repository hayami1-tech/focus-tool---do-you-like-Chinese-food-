...
const App: React.FC = () => {
  ...
  const handleFoodSelect = (food: FoodItem) => {
    if (status === 'RUNNING') {
      if (window.confirm('烹饪正在进行中，确定要换个锅吗？')) {
  ...
  return (
    <div className="min-h-screen bg-[#f3eee3] text-[#4a3728] p-4 md:p-8 flex flex-col items-center">
      <header className="w-full max-w-6xl flex flex-col items-center mb-10 gap-6">
        <h1 className="text-3xl md:text-4xl font-bold tracking-[0.2em] text-[#8b4513] text-center uppercase">Grandma’s Stove</h1>
        <div className="flex flex-col md:flex-row items-center gap-6 w-full justify-between border-t border-[#8b4513]/10 pt-6">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-3 bg-white/40 px-4 py-2 rounded-full border border-[#8b4513]/20 shadow-sm">
               <span className="text-[10px] font-bold uppercase tracking-widest opacity-50">项目:</span>
          ...
          <nav className="flex bg-[#e5dec9] p-1 rounded-full shadow-inner">
            <button onClick={() => setActiveTab('FOCUS')} className={`px-8 py-2.5 rounded-full text-sm font-bold transition-all ${activeTab === 'FOCUS' ? 'bg-[#8b4513] text-white shadow-md' : 'text-[#8b4513]/60'}`}>专注</button>
            <button onClick={() => setActiveTab('HISTORY')} className={`px-8 py-2.5 rounded-full text-sm font-bold transition-all ${activeTab === 'HISTORY' ? 'bg-[#8b4513] text-white shadow-md' : 'text-[#8b4513]/60'}`}>历史</button>
          </nav>
        </div>
      </header>
      ...
      {activeTab === 'FOCUS' ? (
        <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          <div className="lg:col-span-7 flex flex-col items-center justify-start">
            ...
            <div className="mt-8 w-full"><TimerDisplay timeLeft={timeLeft} status={status} onStart={startTimer} onPause={pauseTimer} onReset={() => resetTimer()} onTimeChange={handleManualTimeChange} mode={mode} /></div>
          </div>
          <div className="lg:col-span-5 flex flex-col justify-start">
            <div className="flex justify-center gap-4 bg-[#e5dec9] p-2 rounded-full shadow-inner mb-8">
              <button onClick={() => toggleMode(TimerMode.FOCUS)} className={`px-6 py-2 rounded-full text-sm font-bold ${mode === TimerMode.FOCUS ? 'bg-[#8b4513] text-white' : ''}`}>专注模式</button>
              <button onClick={() => toggleMode(TimerMode.SHORT_BREAK)} className={`px-6 py-2 rounded-full text-sm font-bold ${mode !== TimerMode.FOCUS ? 'bg-[#8b4513] text-white' : ''}`}>休息模式</button>
            </div>
            <Pantry items={mode === TimerMode.FOCUS ? FOOD_ITEMS : BREAK_ITEMS} selectedId={selectedFood.id} onSelect={handleFoodSelect} title={mode === TimerMode.FOCUS ? "选择食材" : "选择零食"} />
          </div>
        </div>
      ) : (
      ...
      {showCatModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#f3eee3] w-full max-w-md rounded-2xl border-4 border-[#8b4513] p-6 shadow-2xl">
            <div className="flex justify-between items-center mb-6"><h2 className="font-bold text-[#8b4513] uppercase tracking-wider">管理分类</h2><button onClick={() => setShowCatModal(false)} className="text-2xl">&times;</button></div>
            <div className="flex gap-2 mb-6">
              <input type="text" value={newCatName} onChange={(e) => setNewCatName(e.target.value)} placeholder="新分类名称..." className="flex-1 px-4 py-2 rounded-lg border-2 border-[#8b4513]/20 text-sm outline-none" />
              <button onClick={addCategory} className="bg-[#8b4513] text-white px-4 py-2 rounded-lg font-bold text-sm">添加</button>
            </div>
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {categories.map(cat => (<div key={cat} className="flex justify-between items-center bg-white/60 p-3 rounded-lg"><span className="font-bold text-sm">{cat}</span><button onClick={() => executeDelete(cat)} className="text-red-600 text-xs font-bold uppercase">删除</button></div>))}
            </div>
          </div>
        </div>
      )}
      ...
      <footer className="mt-20 mb-10 text-center opacity-40 text-xs italic">
        <p>“当我想起中国食物时，我会想起外婆监督我做作业的童年。回到那个没有手机，没有AI的时代。”</p>
      </footer>
    </div>
  );
};
